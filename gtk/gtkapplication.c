...
gtk_application_class_init (GtkApplicationClass *class)
{
  ...
  gtk_application_props[PROP_SESSION_ID] =
    g_param_spec_object ("session-id", NULL, NULL,
                         GTK_TYPE_WINDOW,
                         G_PARAM_READWRITE|G_PARAM_STATIC_STRINGS);
 ...
 * @application: a `GtkApplication`
 * @session_id: The identifier used for session management purposes
 *
 * Sets the identifier used for session management purposes. This identifier
 * would have been typically retrieved in prior runs through
 * [method@GtkApplication.get_current_session_id].
 *
 ...
 * [method@GtkApplication.get_current_session_id] may return a different
 ...
 * for session management registration. See [method@GtkApplication.set_session_id]
 * for more information about it.
 *
 * Returns: The session management identificator
 **/
const char *
 ...
 * is using during execution. This identifier may be saved for
 * future executions of the application to have their window state
 * recovered.
 *
 * This function will return %NULL before [signal@GApplication.startup]
 * and may only return a valid string afterwards on windowing environments
 * where session management is supported. This only the case nowadays
 * for Wayland.
 *
 * Note that this identifier may end up being different from the one
 * specified through [method@GtkApplication.set_session_id], e.g. in
 * the case the specified session ID could not be recovered and a new
 * session was created.
 *
 * Returns: The session identifier to preserve for future runs
 **/
const char *
