...
struct _GdkWaylandToplevel
{
  ...
  struct wp_transaction_v1 *transaction;
};
...
static struct wp_transaction_v1 *
maybe_ensure_transaction (GdkWaylandToplevel *toplevel)
{
  ...
  if (!toplevel->transaction)
    {
      struct wp_transaction_manager_v1 *manager =
        display_wayland->transaction_manager;

      toplevel->transaction =
        wp_transaction_manager_v1_create_transaction (manager);
      wp_transaction_v1_add_surface (toplevel->transaction,
                                             ...
                                             GdkWaylandPopup    *popup)
{
  ...
  wp_transaction_v1_add_surface (transaction,
  ...
  if (GDK_IS_WAYLAND_TOPLEVEL (surface))
    {
      ...
      g_clear_pointer (&toplevel->transaction, wp_transaction_v1_commit);
    }
}
