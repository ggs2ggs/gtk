                     ...
                     GdkColorState *ccs)
{
  ...
  gsk_cairo_set_source_rgba (cr, ccs, &self->color);
                               ...
                               GdkColorState *ccs)
{
  ...
  if (self->stops[0].offset > 0.0)
    gsk_cairo_pattern_add_color_stop_rgba (pattern, ccs, 0.0, &self->stops[0].color);
  for (i = 0; i < self->n_stops; i++)
    {
      gsk_cairo_pattern_add_color_stop_rgba (pattern,
                                            ccs,
                                            self->stops[i].offset,
                                            &self->stops[i].color);
    }
  if (self->stops[self->n_stops-1].offset < 1.0)
    gsk_cairo_pattern_add_color_stop_rgba (pattern, ccs, 1.0, &self->stops[self->n_stops-1].color);
                       ...
                       GdkColorState *ccs)
{
  ...
  if (gdk_rgba_equal (&self->border_color[0], &self->border_color[1]) &&
    ...
    {
      gsk_cairo_set_source_rgba (cr, ccs, &self->border_color[0]);
    }
  else
    {
             ...
             GskBlurFlags          blur_flags)
{
  ...
  gsk_cairo_set_source_rgba (cr, ccs, color);
                     ...
                     GdkColorState *ccs)
{
  ...
  if (gdk_color_state_equal (ccs, GDK_COLOR_STATE_SRGB))
  ...
  else
    {
      ...
      cairo_set_source_surface (cr, self->surface, 0, 0);
      cairo_paint (cr);
      gsk_cairo_surface_convert_color_state (cairo_get_group_target (cr),
                                             GDK_COLOR_STATE_SRGB,
                                             ccs);
      cairo_pop_group_to_source (cr);
      cairo_paint (cr);
  ...
  if (gsk_transform_get_category (self->transform) < GSK_TRANSFORM_CATEGORY_2D)
    {
      ...
      gsk_cairo_set_source_rgba (cr, ccs, &pink);
      gsk_cairo_rectangle (cr, &node->bounds);
      cairo_fill (cr);
      return;
    }
                    ...
                    GdkColorState *ccs)
{
  ...
  if (gsk_render_node_get_node_type (self->child) == GSK_COLOR_NODE &&
    ...
    {
      gsk_cairo_set_source_rgba (cr, ccs, gsk_color_node_get_color (self->child));
      cairo_fill (cr);
    }
  else
    {
  ...
  for (i = 0; i < self->n_shadows; i++)
    {
      ...
      pattern = cairo_pop_group (cr);
      cairo_reset_clip (cr);
      gsk_cairo_set_source_rgba (cr, ccs, &shadow->color);
      cairo_mask (cr, pattern);
      cairo_pattern_destroy (pattern);
      cairo_restore (cr);
